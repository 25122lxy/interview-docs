...
### HashMap的底层数据结构是什么(1.7和1.8有什么区别)✔
...
1、快速失败（fail-fast）： 快速失败是指在进行迭代操作的过程中，**如果集合的结构发生了变化（例如添加或删除元素），那么会立即抛出`ConcurrentModificationException`异常**，以确保在多线程环境下迭代器的一致性。这种策略可以及早地发现并防止潜在的并发修改问题，但不能保证线程安全，因此需要开发者自己处理并发修改的情况。
